// strategies/level_ml_strategy/src/trade_signal_calculator.rs
...
use crate::scorer::{LevelMlScorer, LevelMlScoreBreakdown, SetupKind};
...
impl TradeSignalCalculator {
    ...
    pub async fn build_trade_signal(
        ...
        let breakdown_opt: Option<LevelMlScoreBreakdown> = scorer
        ...
        let breakdown_json = json!({
            ...
            "ml_predictors_score": breakdown.ml_predictors_score,
    ...
    fn calculate_tp_sl(
        ...
        breakdown: &LevelMlScoreBreakdown,
    ) -> (f64, f64, f64, f64) {
        let side_f = side as f64;

        // Use ML price target if available (with sanity check)
        let target_tp1 = if let Some(target) = ml_price_target {
            ...
            if predicted_move * side_f > 0.0 && ml_dist <= max_ml_dist {
                ...
                // ML agrees but target unreasonably far → cap
                entry_price + side_f * max_ml_dist
            } else {
                // ML disagrees → ATR fallback
