// strategies/indicator_heuristic_strategy/src/scorer.rs
//
// Indicator Heuristic Strategy Scorer
//
// Эта стратегия полагается исключительно на:
// 1. Эвристические предсказания (PriceTarget от Hard/Heuristic)
...
/// Final scorer configuration для Indicator Heuristic стратегии
pub struct IndicatorHeuristicScorer {
    min_final_score: f64,
    indicator_weights: IndicatorWeights,
    heuristic_prediction_weight: f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IndicatorHeuristicScoreBreakdown {
    pub final_score: f64,
    pub indicator_score: f64,
    pub heuristic_prediction_score: f64,
...
impl IndicatorHeuristicScorer {
    ...
    pub fn with_heuristic_weight(mut self, weight: f64) -> Self {
        self.heuristic_prediction_weight = weight.clamp(0.0, 1.0);
        self
    }
    ...
    pub async fn score_signal_verbose(
        ...
        raw_signals_summary: &Value,
        predictors: &[PredictionRow],
    ) -> Result<Option<IndicatorHeuristicScoreBreakdown>> {
        let side_i8 = side.signum() as i8;
        
        // ═══════════════════════════════════════════════════════════════
        // 1. Heuristic Prediction Score - ТОЛЬКО эвристические предсказания
        // ═══════════════════════════════════════════════════════════════
        let heuristic_prediction_score = self.calculate_heuristic_prediction_score(predictors);
        ...
        // 4. Final Score
        // ═══════════════════════════════════════════════════════════════
        let base_score = heuristic_prediction_score * self.heuristic_prediction_weight 
                       + indicator_score * (1.0 - self.heuristic_prediction_weight);
        ...
        let debug = json!({
            ...
            "side": side_i8,
            "heuristic_prediction_weight": self.heuristic_prediction_weight,
            "base_score": base_score,
            "heuristic_prediction_score": heuristic_prediction_score,
        ...
        if final_score >= self.min_final_score {
            Ok(Some(IndicatorHeuristicScoreBreakdown {
                final_score,
                indicator_score,
    ...
    /// Calculate heuristic prediction score - uses ONLY heuristic predictions
    fn calculate_heuristic_prediction_score(&self, predictors: &[PredictionRow]) -> f64 {
    ...
    fn infer_setup(&self, raw_signals_summary: &Value, _side: i8) -> (SetupKind, f64) {
        ...
        let is_momentum = adx >= 25.0 && adx < 50.0;
