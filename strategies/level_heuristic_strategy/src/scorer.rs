// strategies/level_heuristic_strategy/src/scorer.rs
// Level Heuristic Strategy - использует ТОЛЬКО эвристические предсказания с уровнями
...
pub struct LevelHeuristicScoreBreakdown {
    pub final_score: f64,
    pub base_score: f64,
    pub heuristic_predictors_score: f64,
...
pub struct LevelHeuristicScorer {
    min_final_score: f64,
    adx_filter_threshold: f64,
}

impl LevelHeuristicScorer {
    ...
    pub async fn score_signal_verbose(
        ...
        raw_signals_summary: &Value,
        predictors: &[PredictionRow],
    ) -> Result<Option<LevelHeuristicScoreBreakdown>> {
        let side_i8 = side.signum() as i8;

        // Heuristic Predictors score - ТОЛЬКО эвристика
        let heuristic_predictors_score = self.calculate_heuristic_predictors_score(predictors);
        ...
        // Base score: 50% heuristic, 30% raw, 20% indicators
        let base_score = heuristic_predictors_score * 0.50 + raw_signals_score * 0.30 + indicators_score * 0.20;

        // Extract level probs from heuristic predictors
        let (bounce_prob, breakout_prob) = self.extract_heuristic_level_probs(predictors);
        ...
        let debug = json!({
            ...
            "heuristic_predictors_score": heuristic_predictors_score,
        ...
        if final_score >= self.min_final_score {
            Ok(Some(LevelHeuristicScoreBreakdown {
                final_score,
                base_score: level_adjusted_score,
                heuristic_predictors_score,
                raw_signals_score,
                indicators_score,
                setup_kind,
                setup_confidence,
    ...
    fn calculate_heuristic_predictors_score(&self, predictors: &[PredictionRow]) -> f64 {
    ...
    fn extract_heuristic_level_probs(&self, predictors: &[PredictionRow]) -> (Option<f64>, Option<f64>) {
        let bounce = predictors.iter()
            .find(|p| p.aspect == PredictionAspect::LevelBounce && p.calc_source == CalcSource::Hard)
            .map(|p| p.value);
        let breakout = predictors.iter()
            .find(|p| p.aspect == PredictionAspect::LevelBreakout && p.calc_source == CalcSource::Hard)
            .map(|p| p.value);
        (bounce, breakout)
    }
