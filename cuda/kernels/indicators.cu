...
// Simple moving average kernel - batch version
    ...
    if (idx < n) {
        if (idx < period) {
            ...
            for (int i = 1; i <= period; i++) {
                if (idx >= i) {  // Prevent out-of-bounds access
...
// Batch version of MACD kernel
__global__ void macd_batch_kernel(
    const double* input,
    double* macd_line,
    double* signal_line,
    double* histogram,
    ...
    if (idx < n) {
        ...
        if (idx >= slow_period) {
            // Calculate fast EMA properly
            double fast_ema = input[idx];
            if (idx > 0) {
                // For early periods, use SMA-like initialization
                if (idx < fast_period) {
                    double sum = 0.0;
                    for (int i = 0; i <= idx; i++) {
                        sum += input[i];
            ...
            // Signal line (EMA of MACD line)
            if (idx >= slow_period + signal_period) {
                // For early signal period, use SMA-like initialization
                if (idx < slow_period + signal_period) {
                    double sum_macd = 0.0;
                    for (int i = slow_period; i <= idx; i++) {
                        sum_macd += macd_line[i];
                    }
                    signal_line[idx] = sum_macd / (idx - slow_period + 1);
                } else {
                    signal_line[idx] = (macd_line[idx] - macd_line[idx-1]) * multiplier_signal + macd_line[idx-1];
                }
