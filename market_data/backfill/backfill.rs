...
// load_active_symbols: возвращает активные символы
pub async fn load_active_symbols(db_url: &str) -> Result<Vec<String>> {
    ...
    let rows = client.query("SELECT symbol FROM market.pairs WHERE is_active = true ORDER BY symbol", &[]).await?;
...
pub async fn run_backfill(cfg: Arc<IngestConfig>) -> Result<(Vec<String>, LastCloseMap)> {
    ...
    let mut stream = stream::iter(tasks)
        .map(|(sym, tf)| {
            ...
            let cfg = cfg.clone();
            let sym_clone = sym.clone(); // Сохраняем копию символа
            let tf_clone = tf; // Сохраняем копию timeframe
            ...
            async move {
                ...
                match klines {
                    Ok(klines) => {
                        ...
                        Ok((sym_clone, tf_clone.as_binance_interval().to_string(), local_last))
                    },
                    Err(e) => {
                        // Логируем ошибку, но не роняем весь процесс, возвращаем None
                        eprintln!("Failed backfill {} {}: {:#}", sym_clone, tf_clone, e);
                        Ok((sym_clone, tf_clone.as_binance_interval().to_string(), None))
                    }
                }
            }
        })
    ...
    let _ = producer.flush(std::time::Duration::from_secs(60));
